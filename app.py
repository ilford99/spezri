...
section[data-testid="stSidebar"] {display:none;} /* optional: hide sidebar completely */
...
components.html(confetti_html, height=0, key="confetti_start")

# --- Floating HTML replay button (no Streamlit widget -> no duplicate IDs) ---
replay_html = """
<style>
.replay {position:fixed; right:18px; bottom:18px; z-index:10;}
.replay button {cursor:pointer; padding:.9rem 1.2rem; border-radius:14px; border:1px solid rgba(255,255,255,.35);
  background:rgba(0,0,0,.35); color:#fff; font-size:1rem; font-weight:800; backdrop-filter: blur(6px);}
.replay button:hover{transform: translateY(-1px); filter:brightness(1.08)}
</style>
<div class=\"replay\"><button onclick=\"(function(){
  const s=document.createElement('script');
  s.src='https://cdn.jsdelivr.net/npm/canvas-confetti@1.9.3/dist/confetti.browser.min.js';
  s.onload=()=>{ 
    const count=280, defaults={ origin:{ y:0.7 } };
    const fire=(r,o)=>confetti(Object.assign({},defaults,o,{particleCount:Math.floor(count*r)}));
    fire(0.25,{spread:46,startVelocity:55});
    fire(0.2,{spread:60});
    fire(0.35,{spread:120,decay:0.91,scalar:1.0});
    fire(0.1,{spread:120,startVelocity:35,decay:0.92});
...
})();\">✨ Noch mehr Konfetti!</button></div>
"""
components.html(replay_html, height=0, key="replay_button")
